 ...
 * @param <T>
 */
public class UndirGraph<T extends Identify> implements Graph<T> {
	...
	public void deleteVertex(T p) {
		...
		int v;
		for (v = 0; v < verNum; v++) {
			if (((Identify)vertexes[v]).getId() == p.getId()) {
				break;
			}
			if (v == verNum - 1) {
